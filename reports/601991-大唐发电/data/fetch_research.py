#!/usr/bin/env python3
"""Fetch Datang Power (601991) research snapshot. Writes JSON/CSV under data/."""
...
def main():
    ...
    TARGET = "601991"
    PEERS = ["601991", "600011", "600027", "600795", "600023"]
    INDEXES = ["sh000300"]

    # 1) quotes
    try:
        quotes = tencent_quote(PEERS + ["sh000300"])
    ...
    for code in PEERS:
        try:
            sina[code] = {
                "lrb": sina_financial_report(code, "lrb", 20 if code == TARGET else 12),
                "fzb": sina_financial_report(code, "fzb", 20 if code == TARGET else 6),
                "llb": sina_financial_report(code, "llb", 20 if code == TARGET else 6),
            }
